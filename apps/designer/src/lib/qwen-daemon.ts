 ...
 *
 * Mirrors the spawn logic in the game's server.js.
 * Module-level singleton — persists across Next.js API route calls in dev mode.
 *
 * Priority:
 *  1. If Qwen is already responding on port 7862 (started by game server), use it.
 *  2. Otherwise, spawn qwen_server.py using the cyod venv and wait for "Listening on".
 */
...
const QWEN_PYTHON = path.join(os.homedir(), 'Documents/cyod/qwen3-tts-venv/bin/python');
const QWEN_SCRIPT = path.join(os.homedir(), 'Documents/cyod/qwen_server.py');
...
/** Kill any stale process occupying QWEN_PORT (e.g. a crashed previous session). */
function freePort(): Promise<void> {
  return new Promise((resolve) => {
    exec(`lsof -ti :${QWEN_PORT} | xargs kill -9 2>/dev/null || true`, () => resolve());
  });
}
