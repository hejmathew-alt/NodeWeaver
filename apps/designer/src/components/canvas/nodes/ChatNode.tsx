...
export function ChatNode({ id, data }: NodeProps) {
  ...
  return (
    <div
      ...
      style={{
        border: isPlaying
          ? `2px solid ${firstLaneColour ?? '#22c55e'}`
          : wasVisited
          ? `1px solid ${firstLaneColour ?? '#22c55e'}66`
          : '1px solid #e2e8f0',
        boxShadow: isPlaying
          ? `inset 4px 0 0 ${firstLaneColour ?? '#22c55e'}, 0 0 20px 6px ${firstLaneColour ?? '#22c55e'}44`
          : `inset 4px 0 0 ${firstLaneColour ?? '#22c55e'}`,
        outline: isSelected ? `2px solid ${firstLaneColour ?? '#22c55e'}` : 'none',
        outlineOffset: '2px',
        animation: isPlaying ? 'nodePulse 1.2s ease-in-out infinite' : undefined,
      }}
    >
      <Handle type="target" position={Position.Top} id="top" style={{ width: 10, height: 10 }} className="!bg-green-400" />
      <Handle type="target" position={Position.Left} id="target-left" style={{ width: 10, height: 10 }} className="!bg-green-400" />
      <Handle type="target" position={Position.Right} id="target-right" style={{ width: 10, height: 10 }} className="!bg-green-400" />
      ...
      <div className="mb-1 flex shrink-0 items-center gap-1.5">
        ...
        {isPlaying && (
          <span className="flex items-end gap-[2px]" style={{ height: 11 }}>
            {[0, 1, 2].map(i => (
              <span key={i} className="w-[3px] rounded-sm bg-green-500"
      ...
      <Handle type="source" position={Position.Bottom} id="bottom" style={{ width: 10, height: 10 }} className="!bg-green-400" />
      <Handle type="source" position={Position.Left} id="left" style={{ width: 10, height: 10 }} className="!bg-green-400" />
      <Handle type="source" position={Position.Right} id="right" style={{ width: 10, height: 10 }} className="!bg-green-400" />
    </div>
  );
}
