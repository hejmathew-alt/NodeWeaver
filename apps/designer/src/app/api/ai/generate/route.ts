...
${WORLD_STEP_SYSTEMS[step as string] ? '' : ''}
...
interface GenerateRequest {
  mode: 'voice' | 'body' | 'line' | 'audio-suggest' | 'sfx-suggest' | 'story-gen' | 'inspire' | 'command-interpret' | 'world-step' | 'world-recycle' | 'avatar-prompt' | 'loom-analyse' | 'loom-chat' | 'lighting-suggest';
  prompt: string;
  anthropicKey: string;
...
export async function POST(req: NextRequest) {
  ...
  const systemPrompt =
    ...
    mode === 'audio-suggest'     ? buildAudioSuggestSystem(context) :
    mode === 'sfx-suggest'       ? buildSfxSuggestSystem(context) :
    mode === 'story-gen'         ? buildStoryGenSystem(context) :
    mode === 'inspire'           ? buildInspireSystem(prompt?.trim() || 'sci-fi') :
    mode === 'command-interpret' ? buildCommandInterpretSystem(context) :
    mode === 'world-step'        ? (WORLD_STEP_SYSTEMS[worldStep] ?? WORLD_STEP_SYSTEMS.locations) :
    mode === 'world-recycle'     ? (WORLD_STEP_SYSTEMS[worldStep] ?? WORLD_STEP_SYSTEMS.locations) :
    mode === 'avatar-prompt'     ? AVATAR_PROMPT_SYSTEM :
    mode === 'loom-analyse'      ? LOOM_ANALYSE_SYSTEM :
    mode === 'loom-chat'         ? LOOM_CHAT_SYSTEM :
    mode === 'lighting-suggest'  ? LIGHTING_SUGGEST_SYSTEM :
  ...
  const userMessage =
    mode === 'voice'
      ...
      : mode === 'audio-suggest'
      ? `Analyze this scene and suggest sound effects, ambient audio, and background music.\n\n${prompt?.trim() || '(no scene text)'}`
      : mode === 'sfx-suggest'
      ? `Suggest sound effects for this block of text:\n\n${prompt?.trim() || '(no text)'}`
      : mode === 'story-gen'
      ? `STORY DESCRIPTION: ${prompt?.trim() || '(no description — generate a generic story for this genre)'}`
      : mode === 'inspire'
      ? (() => {
          const existingTitles: string[] = context?.existingTitles ?? [];
      ...
      : mode === 'world-step'
      ? buildWorldStepUser(context ?? {})
      : mode === 'world-recycle'
      ? buildWorldRecycleUser(context ?? {})
      : mode === 'avatar-prompt'
      ? buildAvatarPromptUser(context)
      : mode === 'loom-analyse'
      ? buildLoomAnalyseUser(context)
      : mode === 'loom-chat'
      ? buildLoomChatUser(context, prompt)
  ...
  const isNonStreaming = ['audio-suggest', 'sfx-suggest', 'story-gen', 'command-interpret', 'world-step', 'world-recycle', 'avatar-prompt', 'loom-analyse', 'lighting-suggest'].includes(mode);
  const maxTokens =
    mode === 'voice'         ? 300 :
    mode === 'line'          ? 200 :
    (mode === 'audio-suggest' || mode === 'sfx-suggest') ? 800 :
    mode === 'story-gen'     ? 16000 :
    mode === 'inspire'       ? 400 :
    mode === 'command-interpret' ? 200 :
    (mode === 'world-step' || mode === 'world-recycle') ? 1200 :
    mode === 'avatar-prompt' ? 200 :
    mode === 'loom-analyse'      ? 1200 :
    mode === 'loom-chat'         ? 400 :
    mode === 'lighting-suggest'  ? 600 :
    500;
